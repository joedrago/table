...
(function() {
  var OWNER_GRACE_PERIOD, SCORE_TIMEOUT, ShuffledDeck, Table, blackoutRank, escapeHtml, prettyCardList, thirteenRank;
  ...
  Table = class Table {
    ...
    playerConnect(pid, socket) {
      ...
      if ((this.owner === pid) && (this.ownerTimeout != null)) {
    ...
    playerDisconnect(pid) {
      ...
      if (pid === this.owner) {
        ...
        if (this.ownerTimeout != null) {
          clearTimeout(this.ownerTimeout);
        }
        this.ownerTimeout = setTimeout(() => {
          return this.findNewOwner();
        }, OWNER_GRACE_PERIOD);
      }
      return this.broadcast();
    }
    ...
    findNewOwner() {
      var pid, player, ref, ref1;
      console.log("Finding new owner...");
      ref = this.players;
      for (pid in ref) {
        player = ref[pid];
        if ((player.socket !== null) && (this.owner === pid)) {
    ...
    logNewScores() {
      ...
      for (pid in ref) {
        player = ref[pid];
        if (this.oldScores[pid] != null) {
          this.log(`Score update (<span class=\"logname\">${escapeHtml(player.name)}</span>): ${this.oldScores[pid]} -> ${player.score}`);
        }
      }
        ...
        case 'setScore':
          if ((this.players[msg.pid] != null) && (msg.pid === this.owner) && (msg.scorepid != null) && (msg.score != null)) {
            if (this.players[msg.scorepid] != null) {
              if (this.oldScores[msg.scorepid] == null) {
                this.oldScores[msg.scorepid] = this.players[msg.scorepid].score;
              }
              if (this.scoreTimeout != null) {
                clearTimeout(this.scoreTimeout);
              }
              this.scoreTimeout = setTimeout(() => {
                return this.logNewScores();
              }, SCORE_TIMEOUT);
