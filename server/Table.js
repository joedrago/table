...
(function() {
  var ShuffledDeck, Table, escapeHtml, prettyCardList, thirteenSortRankSuit;
  ...
  Table = class Table {
    ...
    whoShouldGoFirst() {
      var h, lowestCard, lowestName, pid, player, ref;
      lowestCard = null;
      lowestName = "Unknown";
      ref = this.players;
      for (pid in ref) {
    ...
    deal(template) {
      ...
      switch (template) {
        case 'thirteen':
          ...
          firstPlayer = this.whoShouldGoFirst();
          if (fivePlayer) {
            this.log(`Removed the red 2s and dealt 10 to everyone. <span class=\"logname\">${escapeHtml(firstPlayer)}</span> should go first.`);
          } else {
            this.log(`Dealt 13 to everyone. <span class=\"logname\">${escapeHtml(firstPlayer)}</span> should go first.`);
          }
          this.broadcast();
          break;
        case 'seventeen':
          ...
          firstPlayer = this.whoShouldGoFirst();
          this.log(`Removed the 2 of hearts and dealt 17 to everyone. <span class=\"logname\">${escapeHtml(firstPlayer)}</span> should go first.`);
          this.broadcast();
          break;
        case 'blackout':
          this.mode = 'blackout';
